...
class ShippingAdress {
  elements = {
    ...
    title: () =>
      cy
        .get(":nth-child(1) > .bg-white > .mb-6 > .font-sans")
        .should("have.text", "Shipping Address"),

    inputFirstName: () =>
      cy.getByTestId("shipping-first-name-input").should("be.visible"),
    inputLastName: () =>
      cy.getByTestId("shipping-last-name-input").should("be.visible"),
    inputAddress: () =>
      cy.getByTestId("shipping-address-input").should("be.visible"),
    inputCompany: () =>
      cy.getByTestId("shipping-company-input").should("be.visible"),
    inputPostalCode: () =>
      cy.getByTestId("shipping-postal-code-input").should("be.visible"),
    ...
    inputStateProvince: () =>
      cy.getByTestId("shipping-province-input").should("be.visible"),
    inputPhone: () =>
      cy.getByTestId("shipping-phone-input").should("be.visible"),

    //Shipping - Delivery section
    checkout_visible: () =>
      cy.getByTestId("submit-address-button").should("be.visible"),
    checkout_click: () => cy.getByTestId("submit-address-button").click(),
    //Label and option for Delivery
    title: () =>
      cy
        .get(":nth-child(2) > .bg-white > .mb-6 > .font-sans")
        .should("have.text", "Delivery"),
    title_FakeEx_Standard: () =>
      cy
        .getByTestId("delivery-option-radio")
        .contains("span", "FakeEx Standard")
        .click(),
    title_FakeEx_Express: () =>
      cy
        .getByTestId("delivery-option-radio")
        .contains("span", "FakeEx Express")
        .click(),

    // Cont_Payment
    payment_visible: () =>
      cy.getByTestId("submit-delivery-option-button").should("be.visible"),
    payment_click: () =>
    ...
    title_payment: () =>
      cy
        .get(":nth-child(3) > .bg-white > .mb-6 > .font-sans")
        .should("have.text", "Payment"),
    //verify that rdb is checked TODO
    contToReview_visible: () =>
      cy.getByTestId("submit-payment-button").should("be.visible"),
    contToReview_click: () => cy.getByTestId("submit-payment-button").click(),

    //Review labels and full text
    title_review: () =>
      cy
        .get(":nth-child(4) > .bg-white > .mb-6 > .font-sans")
        .should("have.text", "Review"),

    Review_full: () =>
      cy
        .get(".items-start > .w-full > .font-normal")
        .should(
          "have.text",
